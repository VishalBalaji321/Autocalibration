...
def preview():
    ...
    while ret:
        ...
        for box in boxes:
            ...
            print("Second imshow is passed")
            cv2_imshow(car)
            cv2.imwrite("car.png", car)
            ...
            if not np.isnan(focal) and not np.isinf(m) and not np.isnan(m):
                ...
                cv2_imshow(frame_scale)
                cv2.imwrite("Frame_scale.png", frame_scale)
                ...
                for vp_idx in range(2):
                    for scale_idx, scale in enumerate(scales):
                        ...
                        cv2.imwrite("/content/heatmap_preview_vp{}_s{}.png".format(vp_idx + 1, scale),
                                    cv2.applyColorMap(
